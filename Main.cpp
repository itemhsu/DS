...
void DrawData(cv::Mat mm, cv::Mat frame,
    const std::map < int, DSResult > &map,
    const std::vector < cv::Rect > &outRcs, bool detect)
{
...
void ReadFileContent(const std::string & file,
    std::string & content)
{
...
void CB(cv::Mat & frame, int num)
{
    if (_vw == NULL) {
        //_vw =
        //    new cv::VideoWriter("out.avi", CV_FOURCC('M',
        //        'J', 'P', 'G'), 25.0, cv::Size(frame.cols,
        //        frame.rows));
    }
    if (_rcMap.empty()) {
    ...
    std::vector < cv::Rect > rcs;
    std::map < int, std::vector < cv::Rect >>::iterator it =
        _rcMap.find(num);
    if (it != _rcMap.end()) {
        rcs = it->second;
    }
    ...
    std::map < int, DSResult > map =
        _tt->UpdateAndGet(frame, rcs, num, outRcs);
    int64_t tm1 = gtm();
    Mat mm = frame.clone();
    ...
    if (_isShow) {
        ...
        cv::resize(mm, mm, cv::Size(mm.cols / 2,
                mm.rows / 2));
        cv::resize(frame, frame, cv::Size(frame.cols / 2,
                frame.rows / 2));
        cv::imshow("mm", mm);
...
int main(int argc, char **argv)
{
    if (argc < 2) {
        //printf("usage:\n./tt showornot(0/1)\n");
	std::cout << "usage: DS ./dir \n";
	std::cout << "usage: dir is the directory of MOT16 \n";
	std::cout << "usage: like /home/itemhsu/src/py/deep_sort/MOT16/train/MOT16-02 \n";
        return 0;
    }
    //_isShow = toInt(argv[1]);
    std::cout << "172 _tt->Init \n";
    _isShow = 1;
    _tt = new NT();
    std::cout << "175 _tt->Init \n";
    if (!_tt->Init()) {
        return 0;
    }
    std::cout << "178 _tt->Inited \n";
    std::stringstream ssImgDir;
    ssImgDir << std::string(argv[1]) << "/img1/" ;
    _imgDir = ssImgDir.str().c_str();
    std::cout << "_imgDir = " << _imgDir << "\n";
    std::stringstream ssRcFile;
    ssRcFile << std::string(argv[1]) << "/det/det.txt" ;
    _rcFile = ssRcFile.str().c_str();
    std::cout << "_rcFile = " << _rcFile << "\n";
