...
boost::shared_ptr < NearestNeighborDistanceMetric >
    NearestNeighborDistanceMetric::self_;
    ...
    std::vector < FEATURE > &fts)
{
    ...
    for (int i = 0; i < rcsin.size(); i++) {
        ...
        if (hog == NULL || len != 128) {
            printf
                ("hog(%d) is null or len(%d)!=128,exit!\n",
                hog == NULL, len);
            exit(0);
        }
        FEATURE ft;
    ...
    std::vector < FEATURE > &fts)
{
    ...
    for (int i = 0; i < count; i++) {
        cv::Mat & face = faces[i];
        cv::Rect rc =
            cv::Rect(i * maxw + 5, 5, face.cols, face.rows);
        rcs.push_back(rc);
        cv::Mat tmp = frame(rc);
        face.copyTo(tmp);
    }
...
{
#ifdef UDL
    std::cout << "117 NT::Init\n";
    if (!FeatureGetter::Instance()->Init()) {
    std::cout << "119 NT::Init\n";
        return false;
    }
#endif

    std::cout << "137 NT::Init\n";
    KF::Instance()->Init();
    std::cout << "139 NT::Init\n";
#ifdef UDL
#ifdef UBC
    Mat frame =
        cv::imread("/home/xyz/code1/xyz/img1/000001.jpg");
    std::cout << "144 NT::Init\n";
    ...
    rcs.push_back(rc);
    //}
    std::cout << "166 NT::Init\n";
    ExtractFeature(frame, rcs, fts);
#endif
#endif
    NearestNeighborDistanceMetric::Instance()->Init(0.2,
        100);

    std::cout << "173 NT::Init\n";
    return true;
}

NewAndDelete NT::UpdateDS(const cv::Mat & frame,
    const std::vector < cv::Rect > &rcs, int num,
    const std::vector < int >&oriPos)
{
    ...
    std::cout << num << "----rcs.size():" << rcs.
        size() << "[tm1:" << tm1 << ",tm2:" << tm2 << "(" <<
        (tm2 -
        tm1) << ")" << ",tm3:" << tm3 << "(" << (tm3 -
        tm1) << ")]" << tail.c_str() << "\n";
    return nad;
...
struct FFS {
  public:
    ...
    } void Get(std::vector < std::pair < int,
        FDSSTTrackerP > >&ffs) {
        ffs = ffs_;
    }
  private:
...
// for framebuffer
void NT::UpdateFDSST(const Mat & frame,
    std::vector < cv::Rect > &rcs)
{
...
*/
std::map < int,
    DSResult > NT::UpdateAndGet(const cv::Mat & frame,
    const std::vector < cv::Rect > &rcsin, int num,
    ...
    const std::vector < int >&oriPos)
{
    ...
    resize(ffMat, ffMat, Size(ffMat.cols * scale_,
            ffMat.rows * scale_));
  ...
  for (const auto & track:kalmanTrackers) {
        ...
        printf
            ("trackid:%d, is_confirmed:%d, time_since_update:%d\n",
            id, track->is_confirmed(),
            track->time_since_update_);
        ...
        if (!rcsin.empty()) {
            ...
            printf
                ("id:%d, rc:(%d, %d, %d, %d), oriPos:%d, rcsin.size():%d, rcs.size():%d\n",
                id, rc.x, rc.y, rc.width, rc.height, oriPos,
                rcsin.size(), rcs.size());

        }
        if (!track->is_confirmed()
            || track->time_since_update_ > 0) {
            continue;
        }
